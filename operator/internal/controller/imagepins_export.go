...
// PinnedPostgresImages returns the digest-pinned Postgres images plus the
// optional Pgweb image (sorted for stable subtest ordering), for the multi-arch
...
func PinnedPostgresImages() []string {
	out := make([]string, 0, len(postgresImages)+1)
	for _, img := range postgresImages {
		out = append(out, img)
	}
	out = append(out, pgwebImage)
	slices.Sort(out)
	return out
}
