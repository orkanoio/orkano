...
| `secrets.store-health` | warning | every SecretStore and ExternalSecret in `orkano-apps` reports Ready (ESO's own live validation), every sync's `refreshTime` is within twice its refresh interval, and every target Secret exists | the External Secrets Operator is not installed (enable with `orkano init --secrets-vault`), or it is installed with no stores or syncs configured |
