...
Files 01–05 were hand-written *before* any Go types existed, one per App archetype Orkano must serve; 06 follows the same rule for the Postgres catalog kind (ADR-0014). They are the contract: the CRD schema serves these files, not the reverse. `hack/validate-examples.sh` proves every file is accepted by the generated CRDs in a real cluster.
