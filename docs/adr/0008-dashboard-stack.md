...
- `go:embed` keeps the deployable a single Go binary and keeps Node out of the runtime attack surface; the trade is a rebuild for any UI change (fine, since UI ships with the binary anyway).
