...
Orkano is built by one person in spare time. The project's two highest documented risks are scope creep and the solo bus factor: a decision made at 11pm and never written down is a decision the project loses. The Phase 0 exit criterion is explicit: no open decision may lack an ADR.
...
- Filename: `NNNN-kebab-case-slug.md`. Numbers are assigned when the decision is first proposed; the Phase 0 plan pre-reserved 0001–0008, so gaps and out-of-order dates are expected.
...
- Small overhead on every significant decision; accepted, because the template is deliberately short enough to fill in under ten minutes.
...
- **Decisions in a living planning doc only**: a planning doc states current intent; it cannot carry the why-not-X history without bloating.
