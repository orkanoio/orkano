...
| Tampering | A hostile or hijacked store endpoint feeds poisoned values into app env | TLS to the store; the store spec + credential are admin-authored under step-up | Poisoned values flow into referencing apps — the same power the vault's own admin holds |
