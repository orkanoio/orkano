...
**In scope:** the components Orkano ships — dashboard (React UI + Go API), webhook receiver, operator, build jobs, the in-cluster registry, the Postgres metadata DB, catalog databases, and the optional Mongo Express and Pgweb development UIs.
...
| Elevation | An enabled Mongo Express page abuses the dashboard's browser origin to call unrelated Orkano APIs | Disabled by default; enable/disable require step-up; the proxy strips dashboard credentials before forwarding and applies a path-confined Content Security Policy; Mongo Express has no ServiceAccount token and is reachable only from the dashboard pod | The deprecated upstream UI still processes database-controlled content and can expose or mutate that MongoDB instance — accepted risk #12 |
| Elevation | An enabled Pgweb page abuses the dashboard's browser origin or its database access | Disabled by default; enable/disable require step-up; the proxy strips dashboard credentials and confines browser connections/forms to the Pgweb path; the pod has no ServiceAccount token, is locked to one database, and is reachable only from the dashboard pod | An authenticated Pgweb user can execute SQL and therefore expose or mutate the attached Postgres instance — accepted risk #13 |
...
| Information disclosure | Build exfiltrates cluster credentials, secrets, or another app's uploaded source | No ServiceAccount token mounted, baseline PSA + AppArmor confinement (ADR-0012), default-deny + egress allowlist: DNS, registry, public-internet 443 only — RFC1918, link-local (cloud metadata) and CGNAT (Tailscale) excepted (`config/netpol/`, INV-02), enforcement capability-probed in the M0.5 spike and the substrate smoke. ZIP upload is default-off and its UI warns that the unauthenticated shared registry does not isolate source blobs between build pods | Exfil through the allowed 443 egress (e.g. to an attacker-controlled repo) — accepted risk #2 — or DNS tunneling through CoreDNS — accepted risk #8. If `source.zip` is enabled, a hostile build can enumerate and download retained uploaded sources — accepted risk #14 |
...
12. **Mongo Express is a deprecated, development-only database UI (ADR-0020).** Docker marks the official image deprecated for maintainer inactivity, and upstream warns that database documents can cause server-side JavaScript execution. Orkano pins the last multi-architecture image, disables the feature by default, requires step-up to enable it, keeps it ClusterIP-only behind the authenticated dashboard, gives it no ServiceAccount token, limits its network to its MongoDB pod and DNS, and confines browser connections/forms to its proxy path. Those controls protect the wider cluster and dashboard API; they cannot make the upstream parser trustworthy. Enabling it explicitly accepts that a malicious document can expose or mutate the attached MongoDB instance. Disable it immediately after development use.
13. **Pgweb is a full database administration surface (ADR-0014 amendment).** Orkano pins the maintained MIT-licensed Pgweb 0.17.0 multi-architecture image, disables it by default, requires step-up to enable it, keeps it ClusterIP-only behind the authenticated dashboard, gives it no ServiceAccount token, locks it to the owning database, limits its network to that Postgres pod and DNS, and confines browser connections/forms to its proxy path. These controls isolate the wider cluster and dashboard API; they do not reduce the SQL privileges in the catalog connection Secret. Enabling Pgweb explicitly grants the current Orkano admin the ability to read and mutate that Postgres instance. Disable it after use.
14. **ZIP source blobs are not confidential between build jobs (ADR-0021).** `source.zip` is disabled by default and visibly labeled unsafe. When enabled, archives are digest-addressed, bounded, validated, and reachable only inside Orkano's registry/build network boundary, but the v1 registry is unauthenticated and every hostile build pod needs registry access. A build can therefore enumerate and download any retained uploaded source artifact, then exfiltrate it over permitted public HTTPS or DNS. Do not upload source that must remain secret from other apps on the same single-admin installation. Per-build read authorization is deferred to the registry bearer-token design.
