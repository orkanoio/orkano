...
| STRIDE | Threat | Mitigation | Residual risk |
|---|---|---|---|
| Spoofing | Build pushes an image impersonating another app | Per-build registry credentials; images digest-pinned and cosign-signed; admission verifies signatures (INV-06). Until credentials land, the registry's ingress NetworkPolicy (`config/netpol/`) keeps every pod off the unauthenticated registry except build pods and the operator (digest-resolution manifest HEADs, keyed on its pod label) — enforcement capability-probed in the substrate smoke | One build can still push over another build's tag until per-build credentials land; digest pinning keeps deploys unaffected |
