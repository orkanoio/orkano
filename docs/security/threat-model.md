...
| Information disclosure | Build exfiltrates cluster credentials or secrets | No ServiceAccount token mounted, restricted PSA, egress allowlist: GitHub + registry only (INV-02) | Exfil through the allowed GitHub egress (e.g. to an attacker-controlled repo) — accepted risk #2, granularity pending the M0.5 spike |
...
| Elevation | Malicious Dockerfile escapes the build container to the node | Rootless BuildKit + restricted PSA + no SA token (INV-02); no Docker socket, ever | The big one. Container escape via kernel bug remains plausible; isolation strength pending the M0.5 spike findings (fallbacks: tainted build node pool, gVisor/Kata RuntimeClass) |
