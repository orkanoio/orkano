...
The two `orkano-system` Secret rows are the dashboard's only reach outside `orkano-apps`, and they are deliberately the narrowest the manifest flow (M2.6) can use. The GitHub App onboarding flow exchanges GitHub's manifest code for the App private key and the webhook secret, then writes them where the operator (to mint installation tokens, INV-07) and the receiver (to verify webhook signatures, INV-04) read them — both in `orkano-system`. The grant is `update` only, `resourceNames`-pinned to exactly those two Secrets: not `get` (value-blind, so a compromised dashboard can rotate but never exfiltrate either the App private key or the webhook secret), not `create` (`create` cannot be `resourceNames`-pinned in Kubernetes RBAC, so the install pre-creates an empty `orkano-github-app` placeholder and the flow blind-overwrites it — `orkano-webhook-secret` already exists), and not any other Secret (the superuser password, the dashboard encryption key, and the bootstrap-token hash in `orkano-system` stay unreachable). A separate Role/RoleBinding (`orkano-dashboard-credentials`) carries it, since the namespaced `orkano-dashboard` Role lives in `orkano-apps`.
