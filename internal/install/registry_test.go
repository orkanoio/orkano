...
// TestWireRegistryMasksByAddressFamily pins the ipBlock mask (see hostCIDR).
// The IPv4-mapped case is the subtle one: To4() is non-nil for it, so a
// family test based on that would emit /32 and widen the rule to "::/32".
func TestWireRegistryMasksByAddressFamily(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			nodeIPs: "77.42.93.225 2a01:4f9:c013:1285::1",
			want:    []string{"cidr: 77.42.93.225/32", "cidr: 2a01:4f9:c013:1285::1/128"},
			reject:  "2a01:4f9:c013:1285::1/32",
		},
		{
			name:    "IPv6-only node",
			nodeIPs: "2a01:4f9:c013:1285::1",
			want:    []string{"cidr: 2a01:4f9:c013:1285::1/128"},
			reject:  "2a01:4f9:c013:1285::1/32",
		},
		{
			name:    "IPv4-mapped IPv6 literal",
			nodeIPs: "::ffff:77.42.93.225",
			want:    []string{"cidr: ::ffff:77.42.93.225/128"},
			reject:  "::ffff:77.42.93.225/32",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			n := newFakeRegistryNode()
			n.nodeIPs = tc.nodeIPs
			...
			if strings.Contains(np, tc.reject) {
				t.Errorf("address masked to %q, which authorises far more than the node:\n%s", tc.reject, np)
			}
		})
	}
}
